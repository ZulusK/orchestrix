...
void SpectrumAnalyzer::printError() {
  ...
  case BASS_ERROR_NOCHAN:
    cout << "The sample has no free channels... the maximum number of "
            "simultaneous playbacks has been reached, and no BASS_SAMPLE_OVER "
            "flag was specified for the sample or onlynew = TRUE."
         << endl;
    break;
  case BASS_ERROR_TIMEOUT:
    cout << "The sample's minimum playingTime gap (BASS_SAMPLE) has not yet "
  ...
  case BASS_ERROR_NOTAVAIL:
    cout << "Sample functions are not available when using the no sound"
         << endl;
    break;
  case BASS_ERROR_ILLPARAM:
    cout << "max and/or length is invalid. The length must be specified when "
            "loading from memory."
         << endl;
    break;
  ...
  case BASS_ERROR_CODEC:
    cout << "The file uses a codec that is not available/supported. This can "
            "apply to WAV and AIFF files, and also MP3 files when using the "
            "free BASS version."
         << endl;
    break;
  case BASS_ERROR_FORMAT:
    cout << "The sample format is not supported by the device/drivers. If the "
            "sample is more than stereo or the BASS_SAMPLE_FLOAT flag is used, "
            "it could be that they are not supported."
         << endl;
    break;
  case BASS_ERROR_MEM:
  ...
  case BASS_ERROR_BUFLOST:
    cout << "Should not happen... check that a valid window handle was used "
            "with BASS_Init."
         << endl;
    break;
  }
...
const float *SpectrumAnalyzer::getSpectrums(long ind) const {
  if (ind < 0 || ind >= this->getCount())
    return this->spectrums.at(0)->getSpectrums();
  else
    return this->spectrums.at(ind)->getSpectrums();
}
...
size_t SpectrumAnalyzer::getBars() const { return bars; }

size_t SpectrumAnalyzer::getElementsInChunk() const { return elementsInChunk; }

float SpectrumAnalyzer::getTimeBound() const { return timeBound; }

void *SpectrumAnalyzer::getRawBytes() const { return rawBytes; }

size_t SpectrumAnalyzer::getSamples() const { return samples; }

int SpectrumAnalyzer::getChannels() const { return channels; }

int SpectrumAnalyzer::getFrequency() const { return frequency; }

int SpectrumAnalyzer::getBitsPerSample() const { return bitsPerSample; }
...
float SpectrumAnalyzer::getSHOOT() const { return SHOOT; }

unsigned long SpectrumAnalyzer::getCount() const { return count; }
